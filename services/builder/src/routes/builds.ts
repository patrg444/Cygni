...
const buildsRoutes: FastifyPluginAsync = async (app) => {
  // Create a new build
  app.post<{ Body: BuildRequest }>("/api/builds", async (request, reply) => {
    const { projectId, repoUrl, commitSha, branch, dockerfilePath, buildArgs } = request.body;
  ...
  // Get build details
  app.get<{ Params: BuildParams }>("/api/builds/:buildId", async (request, reply) => {
  ...
  app.get<{ Querystring: BuildQuery }>("/api/builds", async (request, reply) => {
  ...
  // Cancel a build
  app.post<{ Params: BuildParams }>("/api/builds/:buildId/cancel", async (request, reply) => {
    const { buildId } = request.params;
  ...
  app.post<{ Body: { buildId: string; status: BuildStatus; logs?: string; imageUrl?: string } }>(
    "/internal/builds/update",
    async (request, reply) => {
