...
(0, vitest_1.describe)("E2E - Real Kaniko Build with Kind Cluster", () => {
...
- role: control-plane
  ...
  - containerPort: 30000
    ...
    protocol: TCP
`;
                await execAsync(`echo '${kindConfig}' | kind create cluster --name ${CLUSTER_NAME} --config=-`);
                // Create local registry
                await execAsync(`docker run -d --restart=always -p ${REGISTRY_PORT}:5000 --name kind-registry registry:2`);
            ...
            batchApi = kubeConfig.makeApiClient(client_node_1.BatchV1Api);
            coreApi = kubeConfig.makeApiClient(client_node_1.CoreV1Api);
        }
        catch (error) {
    ...
    (0, vitest_1.afterAll)(async () => {
        if (isClusterRunning) {
            // Cleanup namespace
            try {
                await execAsync(`kubectl delete namespace ${NAMESPACE} --timeout=30s || true`);
            }
            catch (e) {
    ...
    (0, vitest_1.it)("should build and push a real Docker image using Kaniko in Kind", async function () {
        ...
        const createResponse = await axios_1.default.post(`${BUILDER_URL}/api/builds`, {
        ...
        for (let i = 0; i < maxAttempts; i++) {
            ...
            try {
                ...
                const kanikoJob = jobList.body.items.find((job) => job.metadata?.name?.includes("kaniko") &&
                ...
                if (kanikoJob) {
                    ...
                    if (kanikoJob.status?.succeeded && kanikoJob.status.succeeded > 0) {
                        ...
                        const podList = await coreApi.listNamespacedPod(NAMESPACE, undefined, undefined, undefined, undefined, `job-name=${kanikoJob.metadata?.name}`);
                        if (podList.body.items.length > 0) {
                            const pod = podList.body.items[0];
                            try {
                                const logs = await coreApi.readNamespacedPodLog(pod.metadata.name, NAMESPACE, undefined, undefined, undefined, undefined, undefined, undefined, undefined, 100);
                ...
                const buildStatus = await axios_1.default.get(`${BUILDER_URL}/api/builds/${apiBuildId}`, {
                    headers: { "X-API-Key": API_KEY },
                });
                console.log(`Build status via API: ${buildStatus.data.status}`);
                if (["success", "failed", "cancelled"].includes(buildStatus.data.status)) {
        ...
        if (jobCompleted) {
            try {
                ...
                const registryResponse = await axios_1.default.get(`http://localhost:${REGISTRY_PORT}/v2/test-project-1/tags/list`);
                console.log("Registry tags:", registryResponse.data);
                (0, vitest_1.expect)(registryResponse.data.tags).toContain(apiBuildId);
        ...
        (0, vitest_1.expect)(jobCompleted || jobFailed).toBe(true);
    }, 300000); // 5 minute timeout
    (0, vitest_1.it)("should handle resource limits and quotas correctly", async function () {
        if (!isKindAvailable || !isClusterRunning) {
            this.skip();
        }
...
`;
        ...
        const jobList = await batchApi.listNamespacedJob(NAMESPACE);
        const runningJobs = jobList.body.items.filter((job) => job.status?.active && job.status.active > 0);
        console.log(`Running jobs: ${runningJobs.length} out of ${builds.length} builds`);
        // With quota limits, not all jobs should run simultaneously
        (0, vitest_1.expect)(runningJobs.length).toBeLessThan(builds.length);
        // Cleanup quota
        await execAsync(`kubectl delete resourcequota e2e-test-quota -n ${NAMESPACE} || true`);
    });
    (0, vitest_1.it)("should clean up failed builds and not leak resources", async function () {
        if (!isKindAvailable || !isClusterRunning) {
            this.skip();
        }
        ...
        const initialCounts = {
            ...
            secrets: initialSecrets.body.items.filter((s) => s.metadata?.name?.includes("kaniko")).length,
        };
        console.log("Initial resource counts:", initialCounts);
        // Create a build that will fail (invalid Dockerfile path)
        const failBuild = await axios_1.default.post(`${BUILDER_URL}/api/builds`, {
            projectId: "test-project-1",
            repoUrl: "https://github.com/GoogleContainerTools/kaniko.git",
            branch: "main",
        ...
        for (let i = 0; i < 30; i++) {
            await new Promise((resolve) => setTimeout(resolve, 5000));
            const status = await axios_1.default.get(`${BUILDER_URL}/api/builds/${buildId}`, {
                headers: { "X-API-Key": API_KEY },
            });
        ...
        (0, vitest_1.expect)(buildFailed).toBe(true);
        ...
        const finalCounts = {
            ...
            secrets: finalSecrets.body.items.filter((s) => s.metadata?.name?.includes("kaniko")).length,
        };
        console.log("Final resource counts:", finalCounts);
        // Resources should be cleaned up (allowing for some active builds)
        (0, vitest_1.expect)(finalCounts.jobs).toBeLessThanOrEqual(initialCounts.jobs + 1);
        (0, vitest_1.expect)(finalCounts.pods).toBeLessThanOrEqual(initialCounts.pods + 1);
        (0, vitest_1.expect)(finalCounts.secrets).toBeLessThanOrEqual(initialCounts.secrets + 1);
    });
});
//# sourceMappingURL=kaniko-real.test.js.map