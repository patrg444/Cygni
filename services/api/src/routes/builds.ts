...
const BUILDER_SERVICE_URL = process.env.BUILDER_SERVICE_URL || 'http://builder:3001';
...
export const buildRoutes: FastifyPluginAsync = async (app) => {
  ...
  app.post('/builds', {
    ...
    if (!project.repository) {
      return reply.status(400).send({ error: 'Project has no repository configured' });
    }
  ...
  app.get('/builds/:buildId', {
    ...
    const hasAccess = await verifyProjectAccess(request.auth!.user.id, build.projectId);
    if (!hasAccess) {
  ...
  app.get('/builds/:buildId/logs', {
    ...
    if (follow) {
      ...
      return reply.status(400).send({ error: 'Use WebSocket endpoint for streaming logs' });
    }

    // Get logs from builder service
    try {
      const response = await axios.get(`${BUILDER_SERVICE_URL}/api/builds/${buildId}/logs`);
  ...
  app.get('/projects/:projectId/builds', {
    ...
    // Verify user has access to the project
    const hasAccess = await verifyProjectAccess(request.auth!.user.id, projectId);
    if (!hasAccess) {
      return reply.status(403).send({ error: 'Access denied' });
    }
  ...
  app.post('/builds/:buildId/cancel', {
    ...
    try {
      ...
      app.log.error('Failed to cancel build in builder service', { buildId, error });
    }
