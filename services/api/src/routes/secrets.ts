...
const createSecretSchema = z.object({
  key: z.string().regex(/^[A-Z_][A-Z0-9_]*$/, 'Key must be uppercase with underscores'),
  value: z.string(),
...
export const secretRoutes: FastifyPluginAsync = async (app) => {
  ...
  app.get('/projects/:projectId/secrets', {
    ...
    const canViewValues = canManageSecrets(request.auth!.organizations[0].role);
...
async function triggerRedeployment(projectId: string, environmentId: string) {
  ...
  console.log(`Triggering redeployment for project ${projectId} in environment ${environmentId}`);
}