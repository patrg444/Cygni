...
(0, vitest_1.describe)("Fullstack Real Deployment", () => {
    let cli;
    let fileSystem;
    let testDir;
    let apiServer;
    ...
    (0, vitest_1.beforeEach)(async () => {
        ...
        fileSystem = new real_file_system_1.RealFileSystem("fullstack-real-deployment-test");
        testDir = await fileSystem.createTestDir();
    });
    (0, vitest_1.afterEach)(async () => {
        await fileSystem.cleanup();
    });
    (0, vitest_1.it)("should perform a complete deployment against test server", async () => {
  ...
  console.log(\`Server running on port \${PORT}\`);
    ...
    `);
        ...
        await fileSystem.createFile("frontend/package.json", JSON.stringify({
            ...
            version: "1.0.0",
            scripts: {
                build: "echo 'Frontend built with API_URL:' && echo $REACT_APP_API_URL",
        ...
        (0, vitest_1.expect)(deployResult.stdout).toContain("Deployment Complete!");
        (0, vitest_1.expect)(deployResult.stdout).toContain("Your application is live at:");
        // Check for live URLs
        (0, vitest_1.expect)(deployResult.stdout).toMatch(/Frontend:\s+https:\/\/app-production\.cloudexpress\.io/);
        (0, vitest_1.expect)(deployResult.stdout).toMatch(/Backend:\s+https:\/\/api-production\.cloudexpress\.io/);
        // Verify deployment manifest was created
        const manifestPath = path_1.default.join(testDir, "deployment-manifest.json");
        (0, vitest_1.expect)(await fileSystem.exists(manifestPath)).toBe(true);
        // Read and verify manifest
        const manifestContent = await fileSystem.readFile("deployment-manifest.json");
        const manifest = JSON.parse(manifestContent);
        (0, vitest_1.expect)(manifest.success).toBe(true);
        (0, vitest_1.expect)(manifest.urls.frontend).toMatch(/^https:\/\/app-production\.cloudexpress\.io$/);
        (0, vitest_1.expect)(manifest.urls.backend).toMatch(/^https:\/\/api-production\.cloudexpress\.io$/);
        (0, vitest_1.expect)(manifest.deployments).toHaveLength(2);
        // Check deployment statuses
        const backendDeployment = manifest.deployments.find((d) => d.service === "backend");
        const frontendDeployment = manifest.deployments.find((d) => d.service === "frontend");
        (0, vitest_1.expect)(backendDeployment.status).toBe("deployed");
        (0, vitest_1.expect)(frontendDeployment.status).toBe("deployed");
    }, 30000); // Extended timeout for deployment
    (0, vitest_1.it)("should handle deployment failures gracefully", async () => {
        ...
        const deployResult = await cli.execute(["deploy:fullstack", "--skip-build"], {
            cwd: testDir,
            env: {
                ...process.env,
                CLOUDEXPRESS_API_URL: `http://localhost:${apiPort}`,
        ...
        (0, vitest_1.expect)(deployResult.code).toBeDefined();
    });
    (0, vitest_1.it)("should support environment-specific deployments", async () => {
        ...
        (0, vitest_1.expect)(buildResult.code).toBe(0);
        // Deploy to staging
        const deployResult = await cli.execute(["deploy:fullstack", "--env", "staging"], {
            cwd: testDir,
            env: {
                ...process.env,
                CLOUDEXPRESS_API_URL: `http://localhost:${apiPort}`,
        ...
        (0, vitest_1.expect)(manifest.urls.backend).toContain("staging");
        (0, vitest_1.expect)(manifest.success).toBe(true);
    }, 30000); // Extended timeout
});
//# sourceMappingURL=fullstack-real-deployment.test.js.map