...
export class LocalStackManager {
   ...
   */
  async start(): Promise<LocalStackCredentials> {
    if (this.isRunning) {
  ...
  async stop(): Promise<void> {
    ...
    try {
      ...
      await execa("docker", ["stop", "localstack-test"]);
    } catch (error) {
    ...
    try {
      await execa("docker", ["rm", "-f", "localstack-test"]);
    } catch {
   ...
   * Create S3 bucket
   */
  async createS3Bucket(bucketName: string): Promise<void> {
   ...
   */
  async createECRRepository(repositoryName: string): Promise<string> {
    const { endpoint, region } = this.getCredentials();
    ...
    for (let i = 0; i < maxAttempts; i++) {
      try {
        const response = await axios.get(
          `http://localhost:${this.port}/_localstack/health`,
        );
 ...
 */
export async function startLocalStack(config?: LocalStackConfig): Promise<{
