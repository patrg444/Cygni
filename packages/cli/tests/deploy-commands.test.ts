...
describe("CLI Deployment Commands", () => {
  describe("cx deploy --aws --dry-run", () => {
    it("should generate CloudFormation template without deploying", async () => {
      ...
      expect(
        template.Resources.TaskDefinition.Properties.ContainerDefinitions[0],
      ).toMatchObject({
        Name: "e2e-test",
        Memory: 512,
        Cpu: 256,
  ...
  describe("cx deploy --aws (real deployment)", () => {
    ...
    it("should deploy to AWS and return service URL", async () => {
      ...
      mockCFClient.send.mockResolvedValueOnce({
        StackId:
          "arn:aws:cloudformation:us-east-1:012178036894:stack/cygni-e2e-test/abc123",
      });
            ...
            Outputs: [
              {
                ...
                OutputValue:
                  "http://e2e-test-lb-123456.us-east-1.elb.amazonaws.com",
              },
            ],
          },
        ],
  ...
  describe("cx deploy --rollback", () => {
    ...
    it("should verify rollback via task definition", async () => {
      ...
      mockECSClient.send.mockResolvedValueOnce({
        taskDefinition: {
          ...
          containerDefinitions: [
            {
              image:
                "012178036894.dkr.ecr.us-east-1.amazonaws.com/cygni-api:v1.0.0",
            },
          ],
        },
      });
      ...
      expect(taskDef.taskDefinition.containerDefinitions[0].image).toContain(
        ":v1.0.0",
      );
    });
  });
