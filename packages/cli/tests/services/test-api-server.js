...
exports.TestApiServer = void 0;
const express_1 = __importDefault(require("express"));
const body_parser_1 = __importDefault(require("body-parser"));
const jsonwebtoken_1 = __importDefault(require("jsonwebtoken"));
const secrets_manager_1 = require("../../src/lib/secrets-manager");
class TestApiServer {
    constructor(useLocalStack = false) {
        this.users = new Map();
        this.tokens = new Map();
        this.organizations = new Map();
        this.projects = new Map();
        this.projectsByOrg = new Map();
        this.environments = new Map();
        this.secrets = new Map();
        this.deployments = new Map();
        this.deploymentLogs = new Map();
        this.userOrganizations = new Map();
        this.cloudExpressDeployments = new Map();
        // Namespace support for preview environments
        this.namespaces = new Map();
        this.namespacedDeployments = new Map();
        this.namespacedDatabases = new Map();
    ...
    seedData() {
        ...
        this.secrets.set("proj_123:env-1", new Map([
            [
                "DATABASE_URL",
                {
                    name: "DATABASE_URL",
    ...
    setupRoutes() {
        ...
        this.app.get("/auth/me", (req, res) => {
            ...
            try {
                ...
                const requiredScope = req.headers["x-required-scope"];
                if (requiredScope) {
        ...
        // Secrets endpoints
        this.app.get("/projects/:projectId/environments/:environmentId/secrets", (req, res) => {
        ...
        this.app.post("/projects/:projectId/secrets", async (req, res) => {
            ...
            if (!environmentId) {
                ...
                const envs = this.environments.get(projectId) || [];
                const envExists = envs.some((e) => e.id === environmentId || e.slug === environmentId);
                if (!envExists) {
                    return res
                        .status(404)
                ...
                const env = envs.find((e) => e.id === environmentId || e.slug === environmentId);
                if (env) {
                    const existingSecrets = this.secrets.get(`${projectId}:${env.id}`) || new Map();
        ...
        this.app.post("/projects/:projectId/environments/:environmentId/secrets", async (req, res) => {
            const { projectId, environmentId } = req.params;
            ...
            if (!/^[A-Z_][A-Z0-9_]*$/.test(name)) {
                return res
                    .status(400)
                    .json({
                    error: "Invalid secret name format. Must start with letter or underscore and contain only uppercase letters, numbers, and underscores.",
                });
            }
            // Check if environment exists
            const envs = this.environments.get(projectId) || [];
            ...
            if (!envExists) {
                ...
                if (env) {
                    ...
                    return this.app._router.handle(Object.assign(req, {
            ...
            const existingSecrets = this.secrets.get(`${projectId}:${environmentId}`) || new Map();
        ...
        this.app.delete("/projects/:projectId/environments/:environmentId/secrets/:secretName", (req, res) => {
        ...
        this.app.get("/builds/cache", (req, res) => {
        ...
        this.app.get("/deployments/:deploymentId/health", (req, res) => {
            res.json({
                healthy: true,
        ...
        this.app.get("/projects/:projectId/deployments/latest", (req, res) => {
            ...
            const sorted = envDeployments.sort((a, b) => new Date(b.createdAt).getTime() - new Date(a.createdAt).getTime());
            const response = {
        ...
        this.app.post("/deployments", (req, res) => {
            ...
            const deployment = {
                id: deploymentId,
                services: cloudexpressConfig.services.map((service) => ({
                    name: service.name,
                    type: service.type,
                    status: "pending",
                })),
        ...
        this.app.post("/deployments/:deploymentId/artifacts", (req, res) => {
            const { deploymentId } = req.params;
            const { serviceName, artifacts } = req.body;
            const deployment = this.cloudExpressDeployments.get(deploymentId);
            if (!deployment) {
                return res.status(404).json({ error: "Deployment not found" });
            }
        ...
        this.app.get("/namespaces", (req, res) => {
    ...
    setUserOrganizations(userId, organizations) {
        this.userOrganizations.set(userId, organizations);
    }
    // Project management
    addProject(organizationId, project) {
    ...
    addEnvironments(projectId, envs) {
        this.environments.set(projectId, envs);
    }
    // Secret management
    async addSecret(projectId, environmentId, name, value) {
        if (this.secretsManager) {
    ...
    async getSecrets(projectId, environmentId) {
        if (this.secretsManager) {
            // Update secretsManager to use the current project ID
            this.secretsManager = new secrets_manager_1.SecretsManager({
                ...this.secretsManager["config"],
    ...
    addDeployments(projectId, deployments) {
        this.deployments.set(projectId, deployments);
    }
    clearDeployments(projectId) {
    ...
    handleNamespacedDeployment(deploymentId, deployment, namespace) {
        // Create namespace if it doesn't exist
        if (!this.namespaces.has(namespace)) {
            const isPreview = namespace.startsWith("preview-");
            const prNumber = isPreview
                ? parseInt(namespace.split("-")[1])
                : undefined;
            const namespaceData = {
                name: namespace,
        ...
        this.namespacedDeployments
            .get(namespace)
            .set(deploymentId, deployment);
    }
    cloneDatabaseForNamespace(namespace) {
    ...
    cleanupNamespace(namespace) {
        const namespaceData = this.namespaces.get(namespace);
        if (!namespaceData)
            return { deployments: 0, databases: 0, services: 0 };
                ...
                setTimeout(() => {
                    ...
                    const interval = setInterval(() => {
                        if (stageIndex < stages.length) {
                            service.status = stages[stageIndex];
                            ...
                            if (service.status === "completed") {
                                ...
                                if (deployment.namespace) {
                                    const namespaceData = this.namespaces.get(deployment.namespace);
                        ...
                        const allCompleted = deployment.services.every((s) => s.status === "completed");
                        if (allCompleted) {
...
exports.TestApiServer = TestApiServer;
//# sourceMappingURL=test-api-server.js.map