...
const RuntimeSpecSchema = z.object({
  ...
  health: z.object({
    ...
    interval: z.string().regex(/^\d+[sm]$/).default("30s"),
    timeout: z.string().regex(/^\d+[sm]$/).default("10s"),
...
export async function validateRuntimeSpec(filePath: string): Promise<ValidationResult> {
  try {
    // Read file
...
export async function detectRuntime(projectPath: string): Promise<RuntimeSpec | null> {
  ...
  for (const name of runtimeOrder) {
    ...
    const detectFiles = Array.isArray(runtime.detect) ? runtime.detect : [runtime.detect];
