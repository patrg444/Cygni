...
export async function deployToCloudExpress(options: DeploymentOptions): Promise<Deployment> {
  ...
  while (buildStatus === 'pending' || buildStatus === 'running') {
    ...
    if (buildStatus === 'running') {
      buildStatusSpinner.text = 'Building container... (this may take a few minutes)';
    }
  }
...
async function ensureProject(api: any, config: BuildConfig) {
  ...
  try {
    ...
    if (error.response?.status === 404) {
      ...
      const projectResponse = await api.post(`/organizations/${orgId}/projects`, {
        name: config.name,
