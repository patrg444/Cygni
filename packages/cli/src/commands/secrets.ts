...
export const secretsCommand = new Command('secrets')
  ...
  .addCommand(
    new Command('set')
      ...
      .option('-e, --env <environment>', 'Environment (leave empty for all environments)')
      .action(async (key, value, options) => {
        try {
          const config = await loadConfig();
          const api = await getApiClient();

          const projectId = await getProjectId(api, options.project || config.name);

          // Validate key format
          if (!/^[A-Z_][A-Z0-9_]*$/.test(key)) {
            console.error(chalk.red('Secret key must be uppercase with underscores (e.g., API_KEY)'));
            process.exit(1);
          }
          ...
          let environmentId;
          if (options.env) {
            const environments = await api.get(`/projects/${projectId}/environments`);
            const env = environments.data.find((e: any) => 
              e.slug === options.env || e.name === options.env
            );
          ...
          if (error.response?.status === 409) {
            console.error(chalk.red('Secret already exists. Use "cygni secrets update" to change it.'));
          } else {
    ...
    new Command('list')
      ...
      .action(async (options) => {
        try {
          ...
          const response = await api.get(`/projects/${projectId}/secrets`, { params });
          ...
          secrets.forEach((secret: any) => {
            const envLabel = secret.environmentId ? chalk.gray(` [${secret.environment?.name || secret.environmentId}]`) : '';
            const value = options.showValues && secret.value 
    ...
    new Command('import')
      ...
      .action(async (file, options) => {
        try {
          ...
          const response = await api.post(`/projects/${projectId}/secrets/bulk`, {
            secrets,
            environmentId,
          });

          const successful = response.data.results.filter((r: any) => r.success).length;
          const failed = response.data.results.filter((r: any) => r.error).length;

          spinner.succeed(`Import complete! ${successful} succeeded, ${failed} failed`);
...
async function getProjectId(api: any, projectNameOrId: string): Promise<string> {
  if (!projectNameOrId) {
