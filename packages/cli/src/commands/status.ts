...
export const statusCommand = new Command('status')
  ...
  .action(async (options) => {
    try {
      ...
      if (!projectId) {
        console.error(chalk.red('No project specified. Use --project or run from a project directory.'));
        process.exit(1);
      }
      ...
      // Get project metrics
      const metrics = await api.get(`/projects/${projectId}/metrics`).catch(() => ({ data: {} }));
      ...
      console.log(chalk.gray(`Organization: ${project.data.organization.name}`));
      ...
      project.data.environments.forEach((env: any) => {
        const latestDeploy = deployments.data.deployments.find((d: any) => 
          d.environment === env.slug
        );
      ...
      if (deployments.data.deployments.length === 0) {
        ...
        deployments.data.deployments.forEach((deploy: any) => {
          ...
          console.log(`  ${statusIcon} ${deploy.id.slice(0, 8)} - ${deploy.environment} - ${time}`);
          console.log(chalk.gray(`     Commit: ${deploy.build.commitSha.slice(0, 7)} (${deploy.build.branch})`));
        });
      }
      ...
      if (metrics.data.requests) {
        ...
        console.log(`  Avg Response Time: ${metrics.data.requests.avgResponseTime || 0}ms`);
      }

      // Quick actions
      console.log(chalk.bold('\n💡 Quick Actions:'));
      console.log(chalk.gray('  Deploy:     ') + chalk.cyan('cygni deploy'));
      console.log(chalk.gray('  View logs:  ') + chalk.cyan('cygni logs --follow'));
      console.log(chalk.gray('  Rollback:   ') + chalk.cyan('cygni deploy --rollback'));
