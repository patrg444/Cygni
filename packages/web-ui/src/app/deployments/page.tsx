...
interface Deployment {
  ...
  status: 'pending' | 'deploying' | 'active' | 'failed' | 'cancelled' | 'rolling-back';
  createdAt: string;
  user: {
    name: string;
    email: string;
...
export default function DeploymentsPage() {
  ...
  const { data: deployments, isLoading } = useQuery({
    ...
    queryFn: () => 
      api.get(`/projects/${selectedProject}/deployments`, {
        params: filters,
      }).then(res => res.data),
  ...
  const statuses = ['pending', 'deploying', 'active', 'failed', 'cancelled', 'rolling-back'];
  ...
  const statusIcons = {
    ...
    'rolling-back': <ReloadIcon className="w-4 h-4 text-orange-500 animate-spin" />,
  };
      ...
      {selectedProject && (
        <>
          ...
          {deployments?.activeStrategies?.length > 0 && (
            <div className="mb-6 space-y-4">
              <h3 className="text-lg font-semibold">Active Deployment Strategies</h3>
                ...
                <DeploymentStrategyView
                  ...
                  onRollback={() => {
                    if (confirm('Are you sure you want to rollback this deployment?')) {
                      api.post(`/deployments/${strategy.deploymentId}/rollback`);
                      queryClient.invalidateQueries({ queryKey: ['deployments'] });
                    }
                  }}
                  onPause={() => {
                  ...
                  >
                    <div className="flex items-start justify-between mb-2">
                      ...
                      <time className="text-sm text-gray-500">
                        {format(new Date(deployment.createdAt), 'MMM d, h:mm a')}
                      </time>
                    </div>
                    ...
                    <div className="space-y-1">
                      <div className="flex items-center gap-2 text-sm">
                        ...
                        <span className="font-medium">{deployment.build.branch}</span>
                      </div>
                      ...
                      {deployment.replicas !== undefined && (
                        <div className="text-sm text-gray-600">
                          Replicas: {deployment.readyReplicas || 0}/{deployment.replicas}
                        </div>
                      )}
                    ...
                    {deployment.canaryStatus?.enabled && (
                      <div className="mt-3">
                        <div className="flex items-center justify-between text-xs text-gray-600 mb-1">
                          <span>Canary: {deployment.canaryStatus.currentWeight}%</span>
                          <span className="capitalize">{deployment.canaryStatus.phase}</span>
                        </div>
                        <div className="w-full bg-gray-200 rounded-full h-2">
                          <div
                            className={cn(
                              "h-2 rounded-full transition-all duration-500",
                              deployment.canaryStatus.phase === 'progressing' && "bg-blue-500",
                              deployment.canaryStatus.phase === 'completed' && "bg-green-500",
                              deployment.canaryStatus.phase === 'failed' && "bg-red-500",
                              deployment.canaryStatus.phase === 'paused' && "bg-yellow-500"
                            )}
                            style={{ width: `${deployment.canaryStatus.currentWeight}%` }}
                          />
                        </div>
                      </div>
                    )}
                      ...
                      <div className="mt-3 p-2 bg-gray-50 rounded">
                        <div className="flex items-center justify-between text-xs mb-1">
                          ...
                          <span className={cn(
                            ...
                            deployment.healthGate.status === 'passing' && "bg-green-100 text-green-700",
                            deployment.healthGate.status === 'failing' && "bg-red-100 text-red-700",
                            deployment.healthGate.status === 'unknown' && "bg-gray-100 text-gray-700"
                          )}>
                        ...
                        {deployment.healthGate.metrics && (
                          <div className="grid grid-cols-3 gap-2 text-xs">
                            <div>
                              ...
                              <div className={cn(
                                "font-medium",
                                deployment.healthGate.metrics.errorRate > 5 ? "text-red-600" : "text-green-600"
                              )}>
                            ...
                            <div>
                              ...
                              <div className={cn(
                                "font-medium",
                                deployment.healthGate.metrics.successRate < 95 ? "text-orange-600" : "text-green-600"
                              )}>
                    ...
                    {deployment.status === 'active' && deployment.endpoint && (
                      <div className="mt-2">
                        <a
                    ...
                    {deployment.status === 'active' && deployment.previousImage && (
                      <div className="mt-2">
                        <button
                          onClick={(e) => {
                            e.stopPropagation();
...
function PreviewEnvironments({ projectId }: { projectId: string }) {
  const { data: previews, isLoading } = useQuery({
    ...
    queryFn: () => 
      api.get(`/projects/${projectId}/preview-environments`).then(res => res.data),
  });
