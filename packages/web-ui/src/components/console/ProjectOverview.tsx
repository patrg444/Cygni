        ...
        <div className="grid grid-cols-4 gap-4 pt-4 border-t">
          <div className="text-center">
            <p className="text-2xl font-bold text-green-600">
              {endpoints.filter((e: any) => e.method === "GET").length}
            </p>
            <p className="text-xs text-gray-600">GET</p>
          </div>
          <div className="text-center">
            <p className="text-2xl font-bold text-blue-600">
              {endpoints.filter((e: any) => e.method === "POST").length}
            </p>
            <p className="text-xs text-gray-600">POST</p>
          </div>
          <div className="text-center">
            <p className="text-2xl font-bold text-yellow-600">
              {endpoints.filter((e: any) => e.method === "PUT").length}
            </p>
            <p className="text-xs text-gray-600">PUT</p>
          </div>
          <div className="text-center">
            <p className="text-2xl font-bold text-red-600">
              {endpoints.filter((e: any) => e.method === "DELETE").length}
            </p>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
          <div>
            ...
            <ul className="space-y-1">
              {data.detectedFiles?.config?.map((file: string) => (
                <li
                  key={file}
          ...
          <div>
            ...
            <ul className="space-y-1">
              {data.detectedFiles?.source?.map((file: string) => (
                <li
                  key={file}
                  className="text-sm text-gray-600 flex items-center gap-2"
                >
