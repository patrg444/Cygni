...
function RuntimeConfig({ initialConfig, framework, }) {
    ...
    (0, react_1.useEffect)(() => {
        if (initialConfig) {
            try {
                const yamlStr = js_yaml_1.default.dump(initialConfig);
                setConfig(yamlStr);
                setIsValid(true);
            }
            catch (error) {
            ...
            const defaultConfig = {
                runtime: framework === "django" || framework === "flask" ? "python" : "node",
            ...
            setConfig(js_yaml_1.default.dump(defaultConfig));
        }
    }, [initialConfig, framework]);
    const getDefaultPort = (fw) => {
        const ports = {
            django: 8000,
            flask: 5000,
            fastapi: 8000,
    ...
    const getDefaultBuildCommand = (fw) => {
        const commands = {
    ...
    const getDefaultStartCommand = (fw) => {
        const commands = {
    ...
    const validateConfig = (value) => {
        const newErrors = [];
        try {
            const parsed = js_yaml_1.default.load(value);
    ...
    const handleSave = async () => {
        ...
        try {
            ...
            console.log("Config saved:", js_yaml_1.default.load(config));
        }
        catch (error) {
    ...
    const resetToDefaults = () => {
        ...
        setConfig(js_yaml_1.default.dump(defaultConfig));
        validateConfig(js_yaml_1.default.dump(defaultConfig));
    };
      ...
      <div className={`p-4 rounded-lg border ${isValid ? "bg-green-50 border-green-200" : "bg-red-50 border-red-200"}`}>
        <div className="flex items-center gap-3">
          {isValid ? (<>
              ...
              <react_icons_1.CrossCircledIcon className="w-5 h-5 text-red-600"/>
        ...
        {errors.length > 0 && (<ul className="mt-2 space-y-1">
            {errors.map((error, index) => (<li key={index} className="text-sm text-red-600 flex items-start gap-2">
      ...
      <div className="bg-white rounded-lg shadow-sm border">
        <div className="flex items-center justify-between px-6 py-4 border-b">
          ...
          <div className="flex items-center gap-2">
            <button onClick={resetToDefaults} className="flex items-center gap-2 px-3 py-1.5 text-sm text-gray-600 hover:text-gray-900 hover:bg-gray-100 rounded-md transition-colors">
              <react_icons_1.ReloadIcon className="w-4 h-4"/>
              Reset to defaults
            </button>
            <button onClick={handleDownload} className="flex items-center gap-2 px-3 py-1.5 text-sm text-gray-600 hover:text-gray-900 hover:bg-gray-100 rounded-md transition-colors">
              <react_icons_1.DownloadIcon className="w-4 h-4"/>
              Download
            </button>
            <button onClick={handleSave} disabled={!isValid || isSaving} className="flex items-center gap-2 px-4 py-1.5 text-sm bg-blue-600 text-white rounded-md hover:bg-blue-700 disabled:opacity-50 disabled:cursor-not-allowed">
              {isSaving ? (<>
        ...
        <div className="relative">
          <textarea value={config} onChange={(e) => handleConfigChange(e.target.value)} className="w-full h-96 p-6 font-mono text-sm bg-gray-50 border-0 resize-none focus:outline-none focus:ring-2 focus:ring-blue-500" spellCheck={false} placeholder="# runtime.yaml configuration"/>
