...
function DeploymentDemo() {
  ...
  const { deployment, isDeploying, isCompleted, isFailed } =
    useDeploymentMonitor(deploymentId);

  // List deployments
  const { data: deploymentsData } = useProjectDeployments("test-project", {
    limit: 5,
  });
    ...
    <div style={{ padding: "20px", fontFamily: "system-ui" }}>
      ...
      <section>
        ...
        <button
          ...
          disabled={createDeployment.isPending || isDeploying}
          style={{
            padding: "10px 20px",
            fontSize: "16px",
            cursor:
              createDeployment.isPending || isDeploying
                ? "not-allowed"
                : "pointer",
      ...
      <section style={{ marginTop: "30px" }}>
        ...
        {deploymentsData?.deployments?.length ? (
          <ul>
            {deploymentsData.deployments.map((deploy) => (
              <li key={deploy.id}>
...
export default function Home() {
  ...
  return (
    <CygniProvider config={{ baseUrl: apiUrl }}>
      <DeploymentDemo />
    </CygniProvider>
  );
}
