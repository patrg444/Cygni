...
const envSchema = z.object({
  ...
  // Feature flags
  ENABLE_MULTI_REGION: z.string().transform(val => val === 'true').default('false'),
  ENABLE_CANARY_DEPLOYMENTS: z.string().transform(val => val === 'true').default('true'),
  FREE_TIER_LIMIT: z.string().transform(val => parseFloat(val)).default('10.00'),
});
