...
class BudgetMonitor {
    ...
    notificationService) {
        ...
        this.FREE_TIER_LIMIT = 10.0; // $10 USD
        this.WARNING_THRESHOLD = 0.8; // Warn at 80% usage
        // Pricing per unit
        this.pricing = {
    ...
    async calculateCurrentUsage(projectId) {
    ...
    async enforceFreeTierLimit(projectId) {
    ...
    async sendBudgetAlert(projectId, usage, type) {
            ...
            await this.notificationService.send({
                ...
                data: {
                    ...
                    usage: {
                        ...
                        limit: this.FREE_TIER_LIMIT,
                        breakdown: {
                            compute: (usage.cpuSeconds * this.pricing.cpuSecond +
                                usage.memoryGBHours * this.pricing.memoryGBHour).toFixed(2),
    ...
    projectExceedTime(dailyRate, currentUsage) {
        if (dailyRate <= 0)
            return undefined;
        const remainingBudget = this.FREE_TIER_LIMIT - currentUsage;
        if (remainingBudget <= 0)
            return new Date(); // Already exceeded
    ...
    async getBudgetStatusForUI(projectId) {
        ...
        return {
            ...
            breakdown: {
                compute: {
                    cost: (usage.cpuSeconds * this.pricing.cpuSecond +
                        usage.memoryGBHours * this.pricing.memoryGBHour).toFixed(2),
