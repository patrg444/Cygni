...
export class StripeService {
  ...
  async createSubscription(teamId: string) {
  ...
  async reportUsage(records: UsageRecord[]) {
    ...
    for (const [teamId, teamRecords] of Object.entries(recordsByTeam)) {
      ...
      if (!team?.stripeSubscriptionId) continue;
  ...
  async handleWebhook(signature: string, payload: string) {
    ...
    switch (event.type) {
      ...
      case 'invoice.payment_failed':
        await this.handlePaymentFailure((event.data.object as Stripe.Invoice).subscription as string);
        break;
  ...
  private async suspendProject(projectId: string) {
  ...
  private groupByTeam(records: UsageRecord[]): Record<string, UsageRecord[]> {
  ...
  private getMetricTypeFromPriceId(priceId: string): string | null {
    for (const [metric, id] of Object.entries(this.PRICE_IDS)) {
      if (id === priceId) return metric;
    }
    return null;
  }

  private mapToStripeMetric(metricType: string): string {
